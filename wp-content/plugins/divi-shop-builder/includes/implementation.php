...
class AGS_Divi_WC_Implementation {
	...
	function ags_divi_wc_fire_customisations()
	{
		...
		$itemActions = array(
			...
			'attribute'           => [
				[ array( $this, 'ags_divi_wc_show_product_attribute' ) ]
			],
			'taxonomy'           => [
				[ array( $this, 'ags_divi_wc_show_product_taxonomy' ) ]
			],
			'row-start'           => [
	...
	function ags_divi_wc_fire_customisation_styles( $body_classes )
	{
		...
		//if ( is_shop() || is_product_taxonomy() || is_product_category() || is_product_tag() ) {
			...
			$ags_divi_wc_body_classes[] = 'ags-divi-wc-percentage-sale-badge-'.(isset( $this->options['sale_percentage_badge_pos'] ) ? $this->options['sale_percentage_badge_pos'] : 'no_overlay');
	...
	function ags_divi_wc_show_product_loop_percentage_sale_badge() {
		...
		if ( $product->is_on_sale() ) {
			...
			if ( $product_sale >= $min_value ) {
				...
				echo '<span class="ags-divi-wc-sale-badge ags-divi-wc-percentage-sale-badge"><span class="onsale">' . str_replace('#',  $product_sale, $text_after_percentage ) . '</span></span>';
			} else {
	...
	/**
	 * Display a product attribute
	 *
	 * @return void
	 */
	function ags_divi_wc_show_product_attribute($params) {
		if (isset($params['attribute'])) {
			$this->_ags_divi_wc_show_product_taxonomy($params['attribute'], isset($params['separator']) ? $params['separator'] : ', ', isset($params['format']) ? $params['format'] : '%s');
		}
	}
	
	
	/**
	 * Display a product taxonomy
	 *
	 * @return void
	 */
	function ags_divi_wc_show_product_taxonomy($params) {
		if (isset($params['taxonomy'])) {
			$this->_ags_divi_wc_show_product_taxonomy($params['taxonomy'], isset($params['separator']) ? $params['separator'] : ', ', isset($params['format']) ? $params['format'] : '%s');
		}
	}
	

	/**
	 * Display a product taxonomy (helper)
	 *
	 * @return void
	 */
	private function _ags_divi_wc_show_product_taxonomy($taxonomy, $separator, $format)
	{
		global $post;
