...
trait FilterHtmlTrait {
			...
			foreach ( $options as $optionId => $optionLabel ) {
				...
				echo '<li'.(empty($children['options']) ? '' : ' class="ags-wc-filters-has-children"').'>
				     ...
				     . (strpos($optionLabel, self::STAR) === false
						? ($isExpandable ? '<a href="#" class="ags-wc-filters-expanded-toggle">' : '').esc_html($optionLabel).($isExpandable ? '</a>' : '')
	...
	function price_display() {
		...
		$this->display_numeric_range( $this->props['show_range'], $this->props['range_min'], $this->props['range_max'], $currencySymbol );
	...
	private function display_numeric_range($style, $min, $max, $currencySymbol=null) {
		...
		if ( $style == 'slider' ) {
			echo(
				'<input type="number" min="' . ((int) $min) . '" max="' . ((int) $max) . '" class="ags-wc-filters-hidden"><input type="number" class="ags-wc-filters-hidden">'
			);
			...
			echo(   '<input type="number" min="' . ((int) $min) . '" max="' . ((int) $max) . '" step="1" title="' . esc_attr__('Minimum value', 'divi-shop-builder') . '">
				</label>
				<span>-</span>
				<label>');
			if ($currencySymbol) {
